...
proxy/                  standalone Caddy (443, auto-TLS) — the SOLE public ingress
  conf/Caddyfile        one vhost per app → <app>-web:3000 over the `edge` network
                        conf/ is bind-mounted as a DIRECTORY at /etc/caddy so a
                        `caddy reload` picks up edits; mounting the file alone
                        pins its inode and silently serves a stale config
