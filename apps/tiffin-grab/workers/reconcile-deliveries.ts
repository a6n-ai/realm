...
// Same self-join alias trick as reconcilePoolFromMisses (deliveries.service.ts): a correlated NOT
...
/**
 * Every order owning a missed original (paused|skipped, cutoff passed) that is not yet pooled and
 * has no legacy make-up. Correctness does not depend on cadence — the write paths (skip/unskip/
 * pause/resume) already call reconcilePoolFromMisses themselves. This just sweeps up anything that
 * missed cutoff without a write happening on it (the common case: nobody touches the row, it just
 * goes stale).
 */
export async function reconcileAllDeliveries(): Promise<number> {
  ...
  for (const { orderId } of rows) {
    pooled += await reconcilePoolFromMisses(orderId);
  }
  log.info({ orders: rows.length, pooled }, "reconciled");
  return pooled;
}
...
// pause/resume) already calls reconcilePoolFromMisses itself, so a missed or delayed run just means
// the sweep-up of untouched rows happens on the next run instead.
