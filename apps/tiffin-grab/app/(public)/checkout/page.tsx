...
export default async function CheckoutPage() {
  ...
  const catalog = toClientCatalog(await loadCatalogSnapshot(await resolveRequestOrg()));
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-4 py-4 sm:py-10">
      <Checkout defaultCountry={defaultCountry} closeHref={closeHref} prefill={prefill} catalog={catalog} />
    </main>
  );
}
