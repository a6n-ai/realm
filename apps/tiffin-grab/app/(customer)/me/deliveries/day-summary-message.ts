...
export function menuNotPublishedCopy(dateIso: string): string {
  return `Menu for ${formatMenuWeekRange(mondayOfIso(dateIso))} isn't published yet`;
}

export function menuNotReleasedCopy(dateIso: string): string {
  return `Menu for ${formatMenuWeekRange(mondayOfIso(dateIso))} isn't released yet`;
}
...
export function selectedDaySummaryMessage({
  ...
  if (kind === "unreleased") return menuNotPublishedCopy(dateIso);

  const status = calendarDayStatus(cell!);
  const released = !!cell!.menuWeekId && (cell!.options.length ?? 0) > 0;
  if (!released && status !== "locked") return menuNotReleasedCopy(dateIso);
