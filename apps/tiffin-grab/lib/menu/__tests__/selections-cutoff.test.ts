...
describe("setSelection per-day cutoff + span", () => {
  beforeEach(async () => {
    ...
    const [u] = await db.insert(users).values({ email: `u${Math.random().toString(36).slice(2)}@test.invalid`,  phone: "+15550000001", name: "T", role: "user" }).onConflictDoNothing().returning();
    ...
    week = w;
    const [d] = await db.insert(dishes).values({ name: "Dal", diet: "veg", active: true }).returning();
