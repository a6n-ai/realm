 ...
 * notification outbox, campaigns) is tracked in notification_outbox itself
 * (status/providerMessageId/lastError) — this used to also write email_log as
 * a second audit trail, retired 2026-09 once every send path went through the
 * outbox. `SES_CONFIGURATION_SET` routes bounce/complaint events to the SNS
 * feedback topic.
 */
