...
describe("audit logging via the intermediate layer (integration)", () => {
  ...
  it("writes a create audit row when a service creates an entity", async () => {
    const inq = await inquiriesService.create({ fullName: "Aud Test", phone: "+16475550000", sourceKey: "manual", email: `inq-${Math.random().toString(36).slice(2)}@test.invalid` });
  ...
  it("writes an update audit row when a service updates an entity", async () => {
    const inq = await inquiriesService.create({ fullName: "Up Test", phone: "+16475550001", sourceKey: "manual", email: `inq-${Math.random().toString(36).slice(2)}@test.invalid` });
    await db.delete(auditLog);
