...
describe("rescheduleDelivery for eatingDays / MWF", () => {
  ...
  it("merges onto an existing trip instead of rejecting", async () => {
    ...
    const wed = all.find((d) => weekdayKey(parseIsoDateUtc(d.deliveryDate)) === "wed" && d.deliveryDate > (mon?.deliveryDate ?? ""));
    expect(mon && wed).toBeTruthy();

    // Move Wed's trip by picking Thu → snaps to Wed — already on Wed; pick a later Mon's Tue onto Wed
    const laterMon = all.filter((d) => weekdayKey(parseIsoDateUtc(d.deliveryDate)) === "mon" && d.deliveryDate > mon!.deliveryDate)[0]
      ?? mon!;
