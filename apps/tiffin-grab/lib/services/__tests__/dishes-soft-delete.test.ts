...
import { attachDishToPlans, testPlanId } from "@/db/test-helpers";
...
describe("dishesService soft-delete", () => {
  beforeEach(async () => {
    await reset();
    const [d] = await db.insert(dishes).values({ planId: await testPlanId(), name: DISH_NAME, category: "sabzi" }).returning();
