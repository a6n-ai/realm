...
/** Mirrors the live layout: title + plan line, week strip (mobile), list + one card (desktop). */
export function DeliveriesSkeleton() {
  return (
    <div aria-busy="true" aria-label="Loading deliveries" role="status">
      ...
      <div className="mb-4 grid grid-cols-7 gap-1.5 lg:hidden">
        {Array.from({ length: 7 }, (_, i) => <Skeleton key={i} className="h-[52px] rounded-[14px]" />)}
      </div>
