...
export function BottomBar({ children, note, className }: { children: ReactNode; note?: string; className?: string }) {
  return (
    <div className={cn(FONT, "c-glass fixed inset-x-0 bottom-0 z-[45] border-t border-[var(--border)] px-4 pt-3", className)} style={{ paddingBottom: "max(0.75rem, env(safe-area-inset-bottom))" }}>
      {note && <p role="status" className="c-caption mx-auto mb-2 max-w-3xl text-center">{note}</p>}
