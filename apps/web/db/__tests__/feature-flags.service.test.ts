...
describe("feature flags service (integration)", () => {
  ...
  it("does not reassign createdAt/createdBy on update", async () => {
    const created = await featureFlagsService.create({ key: "u", label: "U", defaultEnabled: false });
    const updated = await featureFlagsService.update(created.id, {
      defaultEnabled: true,
  ...
  it("rejects an unknown sort field with ValidationError (400)", async () => {
    await expect(
      featureFlagsService.list(undefined, { page: 0, size: 10, sort: { field: "nope", dir: "asc" } }),
