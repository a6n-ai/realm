...
// against the real better-auth instance/DB. xplorers sends auth emails
// directly (no notification_outbox pipeline), so both exports here are
// simple mocks — there's no outbox row to assert against.
...
/**
 * Regression coverage for the two Critical bugs found in tiffin-grab's code
 * review of this same flow, ported here for xplorers:
 ...
 *     runs against the real better-auth instance/DB.
 *
