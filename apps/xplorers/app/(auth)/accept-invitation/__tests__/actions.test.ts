...
describe("acceptInvitationAction", () => {
  it("signs in with the OTP then accepts the invitation with the session cookie from sign-in", async () => {
    ...
    // xplorers-specific: acceptInvitation must bypass the session cookie cache
    // (see actions.ts) so a just-flipped emailVerified isn't read stale.
    expect(acceptCall.query).toEqual({ disableCookieCache: true });
