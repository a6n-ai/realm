...
function MethodCard({
  ...
  return (
    <div className="space-y-4 rounded-xl border bg-muted/30 p-4">
      ...
      <TaxEditor taxes={method.taxes} onChange={(taxes) => onPatch({ taxes })} />
    </div>
  );
}

function TaxEditor({ taxes, onChange }: { taxes: TaxLine[]; onChange: (t: TaxLine[]) => void }) {
  const patchLine = (i: number, p: Partial<TaxLine>) => onChange(taxes.map((t, idx) => (idx === i ? { ...t, ...p } : t)));
  return (
    <div className="space-y-2">
      <p className="text-muted-foreground text-[11px] font-medium tracking-wider uppercase">Taxes</p>
      {taxes.length === 0 ? <p className="text-muted-foreground text-sm">No taxes on this method.</p> : null}
      {taxes.map((t, i) => (
        <div key={i} className="bg-background/80 flex flex-wrap items-end gap-3 rounded-lg p-2">
          <div className="flex flex-col gap-1.5">
            <Label className="text-muted-foreground text-xs">Name</Label>
            <Input value={t.name} placeholder="GST" onChange={(e) => patchLine(i, { name: e.target.value })} className="h-10 w-40" />
          </div>
          <div className="flex flex-col gap-1.5">
          ...
          <Button
            ...
            onClick={() => onChange(taxes.filter((_, idx) => idx !== i))}
      ...
      <Button type="button" variant="outline" size="sm" className="gap-1.5" onClick={() => onChange([...taxes, { name: "", ratePct: 0 }])}>
        Add tax
      </Button>
    </div>
  );
