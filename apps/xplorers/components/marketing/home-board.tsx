...
export async function HomeBoard() {
  const { groups, timeZone } = await loadPublicSessionCards();
  const todayGroup = groups[0];
  const nextGroup = groups[1];
  const tape = todayGroup?.tape ?? formatTapeDay(new Date(), timeZone);
  ...
  return (
    <section className="xpl-blush-band flex flex-col gap-5 border-t border-[var(--rule)] px-5 py-14 lg:grid lg:grid-cols-[4fr_8fr] lg:items-start lg:gap-16 lg:px-20 lg:pt-24 lg:pb-32">
      ...
      <div className="xpl-board-paper relative mt-3 px-4 pt-6 pb-4 shadow-[3px_4px_0_rgba(18,26,36,.12)] lg:mt-0 lg:rotate-[0.4deg] lg:px-9 lg:py-8 lg:shadow-[4px_6px_0_rgba(18,26,36,.12)]">
        <TapeLabel className="top-[-12px] left-4 -rotate-2 lg:top-[-14px] lg:left-9">{tape}</TapeLabel>
        ...
        {todayGroup ? (
          ...
          <p className="m-0 py-8 text-[17px] leading-[1.5]">Nothing on the benches yet. The calendar fills as sessions are published.</p>
        )}
        <div className="flex items-center justify-between pt-3.5 lg:pt-5">
          {nextGroup ? (
            <Link href="/whats-on" className="xpl-mono text-[10px] tracking-[0.12em] lg:text-[11px]">
              {nextGroup.label} →
            </Link>
          ) : (
            <Link href="/whats-on" className="xpl-mono text-[10px] tracking-[0.12em] lg:text-[11px]">
