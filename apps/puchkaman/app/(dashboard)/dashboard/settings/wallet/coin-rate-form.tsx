...
export function CoinRateForm({ current }: { current: CurrentRate }) {
  ...
  const save = () => {
    ...
    start(async () => {
      try {
        await saveCoinRate({ valuePerCoin: n });
  ...
  return (
    <SectionCard
      title="Coin rate"
      subtitle="Sets the CAD value of one coin. Each save adds a new rate record; it does not edit the current one — the latest by date always wins."
    >
