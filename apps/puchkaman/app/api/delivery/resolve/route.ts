...
// same budget as check-address's 20/min.
...
export const POST = handler(async (request: Request): Promise<Response> => {
  ...
  if (isRateLimited(ip, RESOLVE_LIMIT, RESOLVE_WINDOW_MS, "delivery-resolve-ip")) {
  ...
  const place = await resolveAndPersist(parsed.data);
  return json({ place });
});
