    ...
    <form onSubmit={submit} className="card" style={{ background: "var(--white)", padding: "clamp(22px,4vw,34px)" }} noValidate>
      ...
      <div className="grid" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(200px,1fr))", gap: 16 }}>
        ...
        <div className={`field ${errors.event ? "field--err" : ""}`}>
          ...
          {errors.event && <span id={`${formId}-event-error`} className="err-msg">↑ {errors.event}</span>}
        </div>
        <div className="field">
          <label htmlFor={`${formId}-guests`}>Guests</label>
          <select id={`${formId}-guests`} className="select" value={form.guests} onChange={set("guests")}>
            {[1, 2, 3, 4, 5, 6].map((n) => (
