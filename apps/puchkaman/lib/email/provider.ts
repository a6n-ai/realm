...
/**
 * The single SES email provider for the app — auth transactional mail, the
 * catering inquiry form and the notification outbox all send through this.
 * Wrapped so every send (whatever its origin) lands one row in email_log;
 * puchkaman had no audit trail at all before this.
 *
